...
public class Intersection extends Actor {
	...
	public void addLights() {
		for(int dir = 0; dir < lightPlacement.length; dir++) {
			...
			lightPlacement[dir].setRotation(dir * Direction.West.getDirection()); 
			if(lightPlacement[dir].getRotation() == Direction.South.getDirection() || lightPlacement[dir].getRotation() == Direction.North.getDirection()) {
				lightPlacement[dir].setColour(horizontalColour);
				if(lightPlacement[dir].getRotation() == Direction.North.getDirection()) {
				...
				if(lightPlacement[dir].getRotation() == Direction.South.getDirection()) {
					this.getWorld().addObject(lightPlacement[dir], this.getX(), (this.getY() + (Road.ROAD_HEIGHT/2)) + lightPlacement[dir].getImage().getHeight() / 2);
				}
			}
			else {
				if(lightPlacement[dir].getRotation() == Direction.East.getDirection() || lightPlacement[dir].getRotation() == Direction.West.getDirection()) {
					lightPlacement[dir].setColour(verticalColour);
					if(lightPlacement[dir].getRotation() == Direction.East.getDirection()) {
					...
					if(lightPlacement[dir].getRotation() == Direction.West.getDirection()) {
						this.getWorld().addObject(lightPlacement[dir], (this.getX() + (Road.ROAD_HEIGHT/2)) + lightPlacement[dir].getImage().getHeight() / 2, this.getY());
					}
				}
			}
	...
	public void notifyLeaving() {
		for(IntersectionListener car: outerPrevCars) {
			if(!outerCurrCars.contains(car)) {
				car.notifyCarLeaving(this);
			}
		}
