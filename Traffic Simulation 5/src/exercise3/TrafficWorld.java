	...
	public void act() {
		...
		if(randNum == 7) {
			...
			if(randRota == 0) {
				...
				car.setRotation(0);
				car.setLocation(0, HorizontalRoads[randHorz].getY() + Road.ROAD_HEIGHT / 4);
			}
			if(randRota == 1) {
				this.addObject(car, 0, 0);
				car.setRotation(90);
				car.setLocation(VerticalRoads[randVert].getX() - (Road.ROAD_HEIGHT / 4), 0);
			}
			if(randRota == 2) {
				this.addObject(car, 0, 0);
				car.setRotation(180);
				car.setLocation(WORLD_WIDTH, HorizontalRoads[randHorz].getY() - (Road.ROAD_HEIGHT / 4));
				
			}
			if(randRota == 3) {
				this.addObject(car, 0, 0);
				car.setRotation(270);
				car.setLocation(VerticalRoads[randVert].getX() + (Road.ROAD_HEIGHT / 4), WORLD_HEIGHT);
			}
		}
	}
}
